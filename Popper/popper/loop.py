...
class Popper:
    ...
    def run(self, bkcons):
        ...
        if settings.use_memory:
            generator.set_for_memory()
            if os.path.isfile(settings.combiner_path):
                with open(settings.combiner_path, "rb") as f:
                    memory_data = pickle.load(f)
                # Load constraints
                if "constraints" in memory_data:
                    all_cons = memory_data["constraints"]
                    generator.constrain(all_cons)
                # Load combiner state
                if "saved_progs" in memory_data:
                    coverage_pos.update(memory_data["coverage_pos"])
                    coverage_neg.update(memory_data["coverage_neg"])
                    prog_lookup.update(memory_data["prog_lookup"])
                    combiner.saved_progs.update(memory_data["saved_progs"])
                    combiner.inconsistent.update(memory_data["inconsistent"])
                # Reject previous solution if flagged
                if (
                    settings.reject_last
                    and "last_solution" in memory_data
                    and memory_data["last_solution"] is not None
                ):
                    combiner.inconsistent.add(memory_data["last_solution"])
        # @NOTE: Memory -----------------------------------------------------
        ...
        def save_memory():
            ...
            if settings.solution:
                last_solution = frozenset(settings.solution)
            memory_data = {
                "constraints": all_cons,
                "coverage_pos": coverage_pos,
                "coverage_neg": coverage_neg,
                "prog_lookup": prog_lookup,
                "saved_progs": combiner.saved_progs,
                "inconsistent": combiner.inconsistent,
                "last_solution": last_solution,
            }
            with open(settings.combiner_path, "wb") as f:
                pickle.dump(memory_data, f)

        try:
            # Run combiner on loaded programs to find immediate solution
            if settings.use_memory and combiner.saved_progs:
                settings.last_combine_stage = True
                is_new_solution_found = combiner.update_best_prog(set())
            ...
            for size in range(2, max_size + 1):
                ...
                while True:
                    # Cooperative timeout check between C extension calls
                    if time.time() >= settings.deadline:
                    ...
                    if settings.noisy and not skipped:
                        ...
                        if mdl < settings.best_mdl:
                            ...
                            new_cons.extend(
                                self.build_constraints_previous_hypotheses(mdl, prog_size)
                            )
                    ...
                    if (
                        tp > 0
                        and success_sets
                        and (not settings.noisy or (settings.noisy and fp == 0))
                    ):
                        with settings.stats.duration("check subsumed and covers_too_few"):
                            subsumed = pos_covered in success_sets or any(
                                subset(pos_covered, xs) for xs in success_sets
                            )
                            subsumed_by_two = not subsumed and self.check_subsumed_by_two(
                                pos_covered, prog_size
                            )
                            covers_too_few = (
                                not subsumed
                                and not subsumed_by_two
                                and not settings.noisy
                                and self.check_covers_too_few(prog_size, pos_covered)
                            )
                    ...
                    if (
                        not settings.noisy
                        and not has_invention
                        and not is_recursive
                        and (subsumed or subsumed_by_two or covers_too_few)
                    ):
                        ...
                        for subsumed_prog, message in subsumed_progs:
                            ...
                            subsumed_prog_ = frozenset(
                                remap_variables(rule) for rule in subsumed_prog
                            )
                    ...
                    if not settings.noisy and not pruned_more_general:
                        ...
                        if (
                            not inconsistent
                            and settings.functional_test
                            and tp > 0
                            and not pruned_more_general
                        ):
                    ...
                    if settings.noisy:
                        ...
                        if not skipped:
                            ...
                            if spec_size_ <= prog_size:
                                ...
                                len(prog) == 1
                                and spec_size_ < settings.max_body + 1
                                and spec_size_ < settings.max_literals
                            ):
                        ...
                        if skipped or skip_early_neg:
                            ...
                            gen_size_ = min(
                                [
                                    fn + prog_size,
                                    num_pos - fp,
                                    settings.best_mdl - mdl + num_pos + prog_size,
                                ]
                            )
                            if gen_size_ <= prog_size:
                                add_gen = True
                            ...
                            for x in xs:
                                if settings.showcons:
                                    print(
                                        ...
                                        "REDUCIBLE_1:",
                                        "\t",
                                        ",".join(format_literal(literal) for literal in x),
                                    )
                    ...
                    if (
                        not add_spec
                        and not pruned_more_general
                        and settings.datalog
                        and not settings.recursion_enabled
                        and num_neg > 0
                    ):
                    ...
                    if (
                        settings.noisy
                        and not skipped
                        and not skip_early_neg
                        and not is_recursive
                        and not has_invention
                        and tp > prog_size + fp
                        and fp + prog_size < settings.best_mdl
                        and not noisy_subsumed
                    ):
                        local_delete = set()
                        ...
                        if not ignore_this_prog:
                            ...
                            s_pos = set.intersection(
                                *(
                                    covered_by_pos[ex]
                                    for ex, ex_covered_ in enumerate(pos_covered)
                                    if ex_covered_ == 1
                                )
                            )
                        ...
                        if not ignore_this_prog and (inconsistent or fp > 0):
                            ...
                            s_neg = set.intersection(
                                *(
                                    covered_by_neg[ex]
                                    for ex, ex_covered_ in enumerate(neg_covered)
                                    if ex_covered_ == 1
                                )
                            )
                        ...
                        if not inconsistent:
                            ...
                            progs_not_subsumed = set.union(
                                *(
                                    covered_by_pos[ex]
                                    for ex, ex_covered_ in enumerate(not_covered)
                                    if ex_covered_ == 1
                                )
                            )
                            all_progs = set.union(
                                *(
                                    covered_by_pos[ex]
                                    for ex, ex_covered_ in enumerate(tester.pos_examples_)
                                    if ex_covered_ == 1
                                )
                            )
                        ...
                        if not ignore_this_prog:
                            success_sets_noise[(pos_covered, neg_covered)] = (
                                prog,
                                prog_size,
                                fn,
                                fp,
                    ...
                    call_combine = call_combine and (
                        len(to_combine) >= settings.batch_size or size_change
                    )

                    if (
                        add_to_combiner
                        and not settings.noisy
                        and not settings.solution_found
                        and not settings.recursion_enabled
                    ):
                        if any_and(uncovered, pos_covered):
                            ...
                            settings.print_incomplete_solution2(
                                settings.solution, tp, fn, tn, fp, hypothesis_size
                            )
                    ...
                    if call_combine:
                        ...
                        if new_hypothesis_found:
                            ...
                            settings.print_incomplete_solution2(
                                new_hypothesis, tp, fn, tn, fp, hypothesis_size
                            )
                            ...
                            if settings.noisy and best_score < settings.best_mdl:
                                ...
                                new_cons.extend(
                                    self.build_constraints_previous_hypotheses(
                                        settings.best_mdl, prog_size
                                    )
                                )
                                if settings.single_solve:
                    ...
                    if not skipped:
                        if (
                            ...
                            if spec_size <= settings.max_literals and (
                                ...
                                self.seen_hyp_spec[fp + prog_size + mdl].append(
                                    [prog, tp, fn, tn, fp, prog_size]
                                )
                    ...
                    if settings.noisy and not add_gen and gen_size and not pruned_sub_inconsistent:
                        if (
                            gen_size <= settings.max_literals
                            and (settings.recursion_enabled or settings.pi_enabled)
                            and not pruned_more_general
                        ):
                            new_cons.append((Constraint.GENERALISATION, prog, gen_size))
                            self.seen_hyp_gen[fn + prog_size + mdl].append(
                                [prog, tp, fn, tn, fp, prog_size]
                            )
    ...
    def check_redundant_literal(self, prog):
        ...
        for i in range(len(body)):
            ...
            # do it
            out.extend(
                self.check_redundant_literal_aux(
                    new_body, redundant_literal, allsat1, allsat2, depth=0
                )
            )
    ...
    def check_redundant_literal_aux(self, body, literal, allsat1, not_all_sat1, depth):
        ...
        if not connected(body | frozenset([literal])):
            for new_body in combinations(body, len(body) - 1):
                ...
                out.extend(
                    self.check_redundant_literal_aux(
                        new_body, literal, allsat1, not_all_sat1, depth + 1
                    )
                )
            return out
        ...
        for rule in prog:
            ...
            for literal in body:
                ...
                if len(body) == 1:
                    ...
                    if (
                        settings.non_datalog_flag
                        and literal_args.issubset(head_vars)
                        and tester.diff_subs_single(literal)
                    ):
    ...
    def check_covers_too_few_(self, prog_size, pos_covered):
        ...
        # # MAX RULES = 3
            ...
            uncovered = self.tester.pos_examples_ & ~pos_covered
            success_sets = sorted(
                ((pos_covered_, size) for (pos_covered_, size) in self.success_sets.items()),
                key=lambda x: x[1],
            )
            n = len(success_sets)
        ...
        for subprog, unsat_body in unsat_cores:
            ...
            if len(subprog) == 1:
                yield (
                    ...
                    [remap_variables(rule) for rule in subprog],
                )
    ...
    def subsumed_or_covers_too_few(self, prog, seen=set()):
        ...
        for i in range(len(body)):
            ...
            subsumed_by_two = not subsumed and self.check_subsumed_by_two(
                sub_prog_pos_covered, new_prog_size
            )
            covers_too_few = (
                not subsumed
                and not subsumed_by_two
                and self.check_covers_too_few(new_prog_size, sub_prog_pos_covered)
            )
...
def learn_solution(settings):
    ...
    settings.deadline = t1 + settings.timeout
    timeout(
        settings,
        popper,
        (settings, tester, bkcons),
...
def generalisations(prog, allow_headless=True, recursive=False):
    if len(prog) == 1:
        ...
        for i in range(len(prog)):
            ...
            for new_subrule in generalisations(
                [subrule], allow_headless=False, recursive=recursive
            ):
