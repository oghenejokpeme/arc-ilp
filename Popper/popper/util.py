...
def parse_args():
    ...
    parser.add_argument('--use-memory', default=False, action='store_true', help='Use existing constraints for problem if they exist')
    parser.add_argument('--reject-last', default=False, action='store_true', help='Reject the last solution found (use with --use-memory)')
...
class Settings:
    def __init__(self, cmd_line=False, info=True, debug=False, show_stats=True, max_literals=MAX_LITERALS, timeout=TIMEOUT, quiet=False, eval_timeout=EVAL_TIMEOUT, max_examples=MAX_EXAMPLES, max_body=None, max_rules=None, max_vars=None, functional_test=False, kbpath=False, ex_file=False, bk_file=False, bias_file=False, showcons=False, no_bias=False, order_space=False, noisy=False, batch_size=BATCH_SIZE, solver='rc2', anytime_solver=None, anytime_timeout=ANYTIME_TIMEOUT, use_memory=False, reject_last=False):
        ...
        if cmd_line:
            ...
            self.reject_last = args.reject_last
            quiet = args.quiet
            debug = args.debug
        ...
        if self.use_memory:
            self.combiner_path = f'{self.path}/memory.pkl'
